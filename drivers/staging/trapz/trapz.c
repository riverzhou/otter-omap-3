...
static int check_loglevel(const int level, const int cat_id, const int component_id);
...
long systrapz(int ctrl1, int ctrl2, int extra1, int extra2, struct trapz_info __user *ti)
{
	...
	memset(&trigger_event, 0, sizeof(trigger_event));
	...
	spin_lock_irqsave(&g_data.bufLock, flags);
	{
		...
		if (check_loglevel(level, cat_id, comp_id)) {
			// Not filtered by log level, get an entry
			filtered = 0;
	...
	if (filtered) {
		...
		if (ti != NULL) {
			trapz_info_t kti;
			kti.tv = tv;
			kti.sequence = 0;  //TODO Return -1 once we get the kindleprofileperf jar fixed.
...
static int compute_loglevel_buffsize() {
