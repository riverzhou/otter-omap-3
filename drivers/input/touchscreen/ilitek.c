...
//#if resolution_test
		...
		if(tp_id & 0x01)
		{
			...
			input_event(i2c.input_dev, EV_ABS, ABS_MT_WIDTH_MAJOR, 1);*/
            input_event(input, EV_KEY, BTN_TOUCH, 1);
		...
		if (buf[0] == 0 && touch_flag == 1) {
			...
			input_event(i2c.input_dev, EV_ABS, ABS_MT_WIDTH_MAJOR, 0);*/
            input_event(input, EV_KEY, BTN_TOUCH, 0);
            input_mt_sync(i2c.input_dev);
