				...
				enum led_brightness value)
{
	...
	queue_work(led->work_q, &led->work);
}
...
static int omap_pwm_led_probe(struct platform_device *pdev)
{
	...
	led->work_q = create_singlethread_workqueue("pwm_wq");
...
static int omap_pwm_led_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	flush_workqueue(led->work_q);
	printk(KERN_INFO "%s: work flushed\n", __FUNCTION__);
	led_classdev_suspend(&led->cdev);
	printk(KERN_INFO "%s: suspend complete\n", __FUNCTION__);
	return 0;
}
